  ...
  if (userData == null) {
    ...
    return (
      <div className="relative h-screen z-50 ">
        <div className="grid grid-cols-1 md:grid-cols-2 h-full gap-1 px-6 py-0 md:px-12 md:py-28 2xl:px-80 2xl:py-30">
          <div className="w-full h-140 md:h-full flex items-center justify-center">
            <div className="relative flex flex-col bg-white/5 backdrop-blur-lg size-full">
              <div className="absolute inset-x-0 top-16 md:top-10 text-center">
                <div className="text-4xl md:text-5xl font-bold bg-white text-black py-4 md:py-6">Passport</div>
              </div>
              <Image src={IeeeDayLogoOuter} alt="" width={400} height={400} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 scale-75 md:scale-[75%] 2xl:scale-90" />
              <Image src={IeeeDayLogoInner} alt="" width={200} height={200} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 scale-[70%] md:scale-[85%] 2xl:scale-90" />
              <div className="hidden absolute inset-x-0 bottom-0 md:grid md:grid-cols-3 items-center md:mx-10">
                {chapterLogos.map((logo, index) => (<Image key={index} src={logo.src} alt="" height={300} width={300} className={`${logo == IeeeCsLogo ? 'scale-50 md:scale-[65%] 2xl:scale-75' : 'scale-75 md:scale-90 2xl:scale-110'}`} />))}
              </div>
              <div className="md:hidden absolute left-1/2 -translate-x-1/2 -bottom-4 w-full flex items-center justify-center">
                <Image src={shownChapterLogo} alt="" width={300} height={300} className={`${shownChapterLogo == IeeeCsLogo ? "scale-[65%]" : "scale-[85%] mb-8"}`} />
              </div>
            </div>
          </div>
          <div className="hidden fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 md:flex flex-col md:gap-6 2xl:gap-8">
            {[...Array(14),].map((value: undefined, index: number) => (
              <div key={index} className="h-2 w-14 2xl:w-20 border-1 border-white bg-white"></div>
            ))}
          </div>
          <div className="w-full h-140 md:h-full flex items-center justify-center">
            <div className="relative flex flex-col bg-white/5 backdrop-blur-lg size-full">
              <div className="absolute inset-x-0 top-0 md:top-10 flex flex-col items-center justify-center text-center">
                <Image src={IeeeDayLogo2025} alt="" height={300} width={300} className="scale-[70%] md:scale-75 2xl:scale-90" />
                <div className="hidden md:block px-4 md:px-20 mt-2 md:mt-4">A global celebration of technology and innovation, connecting minds, sharing ideas, and shaping the future.</div>
              </div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-full gap-0 px-4 md:px-0">
                <div>
              ...
              <div className="absolute bottom-4 grid grid-cols-2 items-center justify-center w-full text-sm md:text-base px-4 md:px-32 gap-4">
                <div className="border-2 border-white text-center h-24 flex flex-col items-center justify-center">
                  <button className="bg-white text-black px-4 py-2 hover:scale-95 transition-transform duration-300">Game I</button>
                  <div>Will be available in</div>
                  <Countdown date={"2025-10-06T20:00:00Z"} />
                </div>
                <div className="border-2 border-white text-center h-24 flex flex-col items-center justify-center">
                  <button className="bg-white text-black px-4 py-2 hover:scale-95 transition-transform duration-300">Game II</button>
                  <div>Will be available in</div>
